  ...
  return (
    <div className="card p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Channel Parser</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">Parse Discord channel messages and send to webhook</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* Error Display */}
        {error && (
          <div className="flex items-center space-x-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md transition-colors">
        ...
        {/* Success Result */}
        {result && (
          <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md transition-colors">
            <div className="flex items-center space-x-2 mb-2">
              <CheckCircle className="h-4 w-4 text-green-500 dark:text-green-400" />
              <span className="text-sm font-medium text-green-800 dark:text-green-300">Successfully parsed and sent!</span>
            </div>
        ...
        {/* Preview Display */}
        {preview && (
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md transition-colors">
            <h4 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-2">
              Preview: {preview.totalMessages} messages found
            </h4>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {preview.messages.slice(0, 3).map((msg: any, index: number) => (
                <div key={index} className="p-2 bg-white dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600 text-xs transition-colors">
                  <div className="font-medium text-gray-800 dark:text-gray-200">{msg.author.username}</div>
                  ...
                  <div className="text-gray-400 dark:text-gray-500 text-xs">{new Date(msg.timestamp).toLocaleString()}</div>
                </div>
              ))}
