...
export default function EnhancedFirmConfigModal({ firm, onSave, onCancel }: EnhancedFirmConfigModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden transition-colors">
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 180px)' }}>
              ...
              <div>
                ...
                {userGuilds.length === 0 ? (
                  <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                    ...
                    <ul className="text-yellow-700 dark:text-yellow-400 text-xs mt-2 list-disc list-inside">
                    ...
                    {userGuilds.map((guild) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="font-medium text-gray-900 dark:text-white">{guild.name}</h4>
                            <p className="text-sm text-gray-500 dark:text-gray-400">ID: {guild.id}</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {selectedGuild && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors"
                    placeholder="Ex: 123456789012345678"
                  />
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    ID do canal que será monitorado para esta empresa no servidor <strong className="text-gray-900 dark:text-white">{selectedGuild.name}</strong>
                  </p>
                </div>
              )}
            </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Escolha um Template</h3>
              ...
              {/* Component Selection */}
              <div>
                <h4 className="text-md font-medium text-gray-900 dark:text-white mb-3">Componentes Habilitados</h4>
                <div className="space-y-2">
                  {selectedTemplate.config.components.map((component) => (
                    <div key={component.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg transition-colors">
                      <div>
                        <h5 className="font-medium text-gray-900 dark:text-white">{component.name}</h5>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Ordem: {component.order + 1}</p>
                      </div>
              ...
              {/* Theme Customization */}
              <div>
                <h4 className="text-md font-medium text-gray-900 dark:text-white mb-3">Personalização do Tema</h4>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {formData.allowedRoles.map((roleId) => (
                    <div key={roleId} className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg transition-colors">
                      <span className="text-sm text-gray-900 dark:text-white">@{getRoleName(roleId)}</span>
                      <button
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="w-full px-3 py-2 text-left bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 flex items-center justify-between transition-colors"
                    >
                      <span className="text-gray-500 dark:text-gray-400">Adicionar cargo...</span>
                      ...
                      <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                        {discordRoles
                          ...
                          <button
                            ...
                            className="w-full px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-600 flex items-center space-x-2 text-gray-900 dark:text-white transition-colors"
                          >
                            <div
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Revisar Configurações</h3>

              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-4 transition-colors">
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white">Informações Básicas</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Nome: {formData.name}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Servidor: {selectedGuild?.name}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Channel ID: {formData.channelId}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white">Template</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Tipo: {selectedTemplate.name}</p>
                ...
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white">Permissões</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">Endpoint: {formData.monitoring.endpointType}</p>
                </div>
              </div>
            </div>
          )}
