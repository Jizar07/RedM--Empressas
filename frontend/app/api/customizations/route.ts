...
const CUSTOMIZATIONS_FILE = path.join(process.cwd(), '..', 'data', 'custom_display_names.json');
...
async function readCustomizations() {
  try {
    const fileContent = await fs.readFile(CUSTOMIZATIONS_FILE, 'utf-8');
...
async function writeCustomizations(data: any) {
  try {
    const updatedData = {
      ...data,
      ultima_atualizacao: new Date().toISOString()
    };
    await fs.writeFile(CUSTOMIZATIONS_FILE, JSON.stringify(updatedData, null, 2));
    return true;
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Write back to file
    const writeSuccess = await writeCustomizations(customizations);
